...
//game state enum
    ...
    public class LevelManager : MonoBehaviour
    {
        ...
        public Items.Item lastDraggedItem;
        //item which was switched succesfully recently 
        public Items.Item lastSwitchedItem;
...
// #endif
            ...
            if (gameStatus == GameState.Playing || gameStatus == GameState.Tutorial)
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    if (hit != null)
                    {
                        lastTouchedItem = hit.gameObject.GetComponent<Items.Item>();
                ...
                else if (Input.GetMouseButton(1))
                {
                    ...
                    if (hit != null)
                    {
                        var item = hit.gameObject.GetComponent<Items.Item>();
...
//            DestroyItems(true);
        ...
        private IEnumerator RegenMatches(bool onlyFalling = false)
        {
            ...
            var combs = new List<List<Items.Item>>();
            do
            {
        ...
        public void DestroyItems(bool withoutEffects = false)
        {
            ...
            foreach (var item in items)
            {
                if (item != null)
                {
                    if (item.GetComponent<Items.Item>().currentType != ItemsTypes.INGREDIENT &&
                        item.GetComponent<Items.Item>().currentType == ItemsTypes.NONE)
                    {
                        if (!withoutEffects)
                            item.GetComponent<Items.Item>().DestroyItem();
                        else
                            item.GetComponent<Items.Item>().anim.SetTrigger("disappear");
                    }
                }
            }
        }
        ...
        private Items.Item lastTouchedItem;
...
//        Debug.Log("<-next turn->");
        ...
        public List<Items.Item> GetRow(int row)
        {
            var itemsList = new List<Items.Item>();
            for (var col = 0; col < levelData.maxCols; col++)
            {
        ...
        public List<Items.Item> GetColumn(int col)
        {
            var itemsList = new List<Items.Item>();
            for (var row = 0; row < levelData.maxRows; row++)
            {
        ...
        /// <param name="square"></param>
        /// <returns></returns>
        public List<Items.Item> GetItemsAround9(Square square)
        {
        ...
        public List<Items.Item> GetItemsAround8(Square square)
        {
            var col = square.col;
            var row = square.row;
            var itemsList = new List<Items.Item>();
            for (var r = row - 1; r <= row + 1; r++)
            {
                for (var c = col - 1; c <= col + 1; c++)
                {
                    ...
                    if(hit && hit.transform)
                        itemsList.Add(hit.transform.GetComponent<Items.Item>());
                }
            }
