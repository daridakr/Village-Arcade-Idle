...
namespace SweetSugar.Scripts.System
{
    public class ItemMonoBehaviour : MonoBehaviour
    {
        ...
        {
            var item = GetComponent<Items.Item>();
        ...
        IEnumerator DestroyDelay(Items.Item item)
        {
            ...
            if (LevelManager.THIS.DebugSettings.DestroyLog)
                DebugLogKeeper.Log(name + " dontDestroyOnThisMove " + item.dontDestroyOnThisMove + " dontDestroyForThisCombine " + gameObject.GetComponent<Items.Item>()
                                       .dontDestroyForThisCombine,  DebugLogKeeper.LogType.Destroying);
            if (item.dontDestroyOnThisMove || gameObject.GetComponent<Items.Item>().dontDestroyForThisCombine)
            {
                GetComponent<Items.Item>().StopDestroy();
                yield break;
            }
            if (LevelManager.THIS.DebugSettings.DestroyLog)
        ...
        private void OnDestroyItem(Items.Item item)
        {
