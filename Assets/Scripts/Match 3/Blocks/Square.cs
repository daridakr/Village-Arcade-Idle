...
//TODO: Add new target
    ...
    public class Square : MonoBehaviour, ISquareItemCommon, IField, IMarmaladeTargetable
    {
        ...
        public Items.Item GenItem(bool falling = true, ItemsTypes itemType = ItemsTypes.NONE, int color = -1, ItemForEditor squareBlockItem=null)
        {
            if (IsNone() && !CanGoInto())
                return null;
            ...
            else
            {
                ...
                item?.GetComponent<Items.Item>().anim?.SetTrigger("bonus_appear");
            }
            if (item == null)
            {
            ...
            Items.Item itemComponent = item.GetComponent<Items.Item>();
...
//            itemComponent.StartFalling();
        ...
        private GameObject GetIngredientItem()
        {
            ...
            var spawnAmount = selectedTarget.targetPrefab.GetComponent<Items.Item>().SpawnAmount.SpawnAmount;
                ...
                if (!IsBottom())
                {
                    if (restCount - fieldCount > 0)
                    {
                        if (Random.Range(0, LevelManager.THIS.levelData.limit / 3) == 0)
                        {
                            ...
                            item.GetComponent<Items.Item>().sprRenderer[0].sprite = selectedTarget.extraObject;
                        }
                    }
                }
            }
...
//        if (!nextSq?.IsNone() ?? false)
        ...
        /// </summary>
        /// <returns></returns>
        public Items.Item GetItemAbove()
        {
        ...
        public List<Items.Item> FindMatchesAround(FindSeparating separating = FindSeparating.NONE, int matches = 3, Hashtable countedSquaresGlobal = null)
        {
            var globalCounter = true;
            var newList = new List<Items.Item>();
            if (countedSquaresGlobal == null)
            {
                globalCounter = false;
                countedSquaresGlobal = new Hashtable();
            ...
            foreach (DictionaryEntry de in countedSquares)
            {
                newList.Add((Items.Item)de.Value);
            }
